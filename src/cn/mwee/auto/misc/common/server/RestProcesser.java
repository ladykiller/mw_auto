...
import cn.mwee.auto.misc.common.server.resp.ErrorResp;
...
public class RestProcesser extends Object{
	...
	private final void doHttpResp(NettyHttpRequest req,Resp response, final HttpRequest request,final Channel channel, String contentType){
		...
		defaultResponse.headers().set("Content-Type", contentType);
		defaultResponse.headers().set("Content-Length", defaultResponse.getContent().readableBytes());
		boolean close = !HttpHeaders.isKeepAlive(request);
//		close = true;
		defaultResponse.headers().set(HttpHeaders.Names.CONNECTION, close ? HttpHeaders.Values.CLOSE : HttpHeaders.Values.KEEP_ALIVE);
